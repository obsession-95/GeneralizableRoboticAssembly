...
class Search_Env():
    def __init__(self, baseName, objectName):
        ...
        if self.isGRU:
            ...
            self.observation_space = np.zeros(11)

        # 初始化环境参数
        self.steps = 0
        self.relative_pos = None

        if self.objectName == 'circlepeg' or self.objectName == 'sixpeg':
            self.isBinary = True
        else:
            self.isBinary = False
        # 工件图像处理参数
        ...
        if SIM:
            self.img_dicts = {
                'circlepeg':[100, 1646, 49, 1648, 53],
                'sixpeg':[100, 1828, 17, 1850, 54],
                'dualpeg':[100, 524, 21, 574, 20]
            }

            self.clip_dicts = {
                'circlepeg':[75, 160, 70, 190],
            ...
            self.norm_params = {
                # Normalize mean std
                'circlepeg':[0.9725, 0.9725, 0.9725, 0.1574, 0.1574, 0.1574],
                'sixpeg':[0.9725, 0.9725, 0.9725, 0.1574, 0.1574, 0.1574],
                'dualpeg':[0.9725, 0.9725, 0.9725, 0.1574, 0.1574, 0.1574]
            }

            vision_model_path = './model/VisionGuide search/model_search_lite.pth'
        if REAL:
            base_pos_dicts = {
            ...
            if self.isBinary:
                ...
                vision_model_path = './model/VisionGuide search/model_search_real' + '.pth'
        
        self.effective_area_x = self.img_dicts[self.objectName][1] - self.img_dicts[self.objectName][2]
        self.effective_area_y = self.img_dicts[self.objectName][3] - self.img_dicts[self.objectName][4]

        # 初始化视觉分类网络
        # model = MobileNetV3Large(num_classes=2)
        model = MobileNetV3Lite(num_classes=2)
        model.load_state_dict(torch.load(vision_model_path))
        self.model = model.eval().to(device)


    def reset(self, theta=math.pi, isRegular=False):
        self.steps = 0
        self._reset(err_theta=theta, isRegular=isRegular)
    ...
    def step(self, action=None, isProcess=False, isCombination=False):
        ...
        state = self._get_state()
        direct, area, ft = state[1:3], state[3:5], state[5:11]
        relative_pos = self.get_relative_pos()
        # 定义动作选择矩阵

        if SIM:
            s_rz = 1 if isProcess else -self.sign_judge(relative_pos[5])
        elif REAL:
            s_rz = 0
        ...
        if ft[2] > -0.5:
            ...
            s = [direct[0], direct[1], 0, 0, 0, s_rz]

        if isCombination is False:
            action_net = np.copy(action)
            a = self.action_transform(action)

            delta_a = [a[0], a[1], a[2], 0, 0, a[3]]
            # print(f'---action = {delta_a}')      
        else:
            action_net = None
            delta_a = np.copy(action)
        
        if self.objectName == 'usb':
            delta_a[0] = max(delta_a[0], 0.0001)
            delta_a[1] = max(delta_a[1], 0.0001)
        ...
        # time.sleep(0.5)
        state = self._get_state()
        _, area, ft = state[1:3], state[3:5], state[5:11]
        # print(area)
        ...
        reward -= 5*(area[0] + area[1])                            # 用面积表示相对位姿
        ...
        # print(np.linalg.norm(area, ord=np.inf))
        # if (tcp_pos[2] < self.initial_pz - 1e-3) or (np.linalg.norm(area, ord=np.inf) < 5e-3):
        if (tcp_pos[2] < self.initial_pz - 1e-3) or (np.linalg.norm(area, ord=np.inf) < 5e-3) or ( tcp_pos[2] < self.initial_pz - 5e-4 and abs(ft[2] < 0.5)):
        # if (tcp_pos[2] < self.initial_pz - 1e-3)  or (np.linalg.norm(area, ord=np.inf) < 1e-5):
            success = True
            done = True
            reward += 150
    ...
    def first_grasp(self):
        if SIM:
            ...
            tcp_offset_dicts = {
                ...
                'plug3':[0, 0, 0, 351.00*1e-3, 0, 0, 0]
            }
            
            move_pos = copy.deepcopy(self.reset_pos)
            move_pos[2] += 0.05
        ...
        for _ in range(10):
            ...
            for i in range(50):
                ...
                if SIM:
                    ...
                    img_y = sim_rob.getVisionImage(cameraName='Camera_y', clipArr=clip_param)
                    ...
                    # cv2.imshow("binary_img_x", img_x)
                    # cv2.imshow("binary_img_y", img_y)
                ...
                net_img_y, black_area_y = self.img_processing(img_y)

                net_img = [net_img_x, net_img_y]
                label = [0, 0]
                
                for i in range(2):
                    output = self.model(net_img[i])  # 输入网络
                ...
                print(label)
                act = [1e-3, 1e-3]
                a = np.multiply(act, label)
                delta_a = [a[0], a[1], 0, 0, 0, 0]
    ...
    def get_img_state(self):
        ...
        norm_area_y = (black_area_y - self.img_dicts[self.objectName][4])/self.effective_area_y

        img_state = label + [norm_area_x, norm_area_y]
        return img_state
    ...
    def get_relative_pos(self):
        ...
        if SIM:
            ...
            tcp_pos = real_rob.getPosOrt()
            base_pos = copy.deepcopy(self.reset_pos)
            relative_pos = np.array(tcp_pos) - np.array(base_pos)
            # print(f'tcp_ort = {tcp_pos[5]}, base_pos = {base_pos[5]}')
    ...
    def _reset(self, err_theta=math.pi, err_r=3e-3, isRegular=False):
        "回合重置"
        # region reset
        initial_pos = np.copy(self.reset_pos)
        # 引入初始位置误差
        if isRegular:
            theta = uniform(err_theta-math.pi/18, err_theta+math.pi/18)
        if not isRegular:
        ...
        if SIM:
            ...
            relative_pos = self.get_relative_pos()
            # initial_pos[5] += choice([-5/RAD2DEG, 5/RAD2DEG])
            initial_pos[5] += uniform(-5/RAD2DEG, 5/RAD2DEG)
            ...
            initial_pos[2] -= (relative_pos[2] - uniform(0, 3e-4))
            real_rob.setPosOrt(initial_pos)

        
        while True:
            self.ft = self.get_ft()
            tcp_pos = self.get_tcp_pos()
            # print(f'---tcp_pos = {tcp_pos[2]}, reset_pos = {self.reset_pos[2]}, ft = {self.ft}')

            if (self.ft[2] < -1) or (tcp_pos[2] < self.reset_pos[2] - 5e-4):
                self.initial_pos = tcp_pos[0:3]
            ...
            if SIM:
                ...
                real_rob.setPosOrt(initial_pos, speed=0.5, acc=0.25, asynchronous=True, isSoft=True)
        
        # print(f'reset fz = {self.ft[2]}')

    
    def action_guide(self, ft, relative_pos):
        """
        动作引导
        """
        action = self.action_space      # 动作初始化
        # x、y动作大小引导
        norm_pos = 1000*relative_pos[0:3]
        for i in range(2):
            if abs(norm_pos[i]) > 1:
                action[i] = uniform(0.9, 1.0)
            else:
                action[i] = abs(norm_pos[i])
        
        # z方向动作大小引导
        if ft[2] > -0.5:
            # z向下移动
            if np.linalg.norm(relative_pos[0:2], ord=np.inf) < 25e-4:
                action[2] = uniform(0.3, 0.5)
            else:
                action[2] = uniform(0, 0.3)   
        else:
            action[2] = uniform(0.5, 1.0)

        if abs(relative_pos[5]) > 1/RAD2DEG:
            action[3] = uniform(0.9, 1.0)
        else:
            action[3] = abs(relative_pos[5])
    ...
    def action_transform(self, action, isGuide=False):
        ...
        if not isGuide:
            ...
            action[0:3] = action[0:3]*0.001
            action[3] = action[3]/RAD2DEG

        if isGuide:
            # for i in range(4):
            #     action[i] = action[i]*2 - 1
            # action[0:3] = action[0:3]*1000
            # action[3] = action[3]*RAD2DEG
            for i in range(4):
                if action[i] < 0.1:
                    action[i] = 0

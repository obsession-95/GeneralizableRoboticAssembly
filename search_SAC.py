...
from envs.search_env_PG import Search_Env
...
def main():
    ...
    args = get_args()
    # 初始化环境
    env = Search_Env(baseName='sixbase', objectName='sixpeg')
            ...
            for steps in range(test_steps):
                ...
                if LOG:
                    if args.isGRU:
                        ...
                        writer.add_scalar('fx', s[5], global_step=total_step + steps)
                        writer.add_scalar('fy', s[6], global_step=total_step + steps)
                        writer.add_scalar('fz', s[7], global_step=total_step + steps)
                        writer.add_scalar('tx', s[8], global_step=total_step + steps)
                        writer.add_scalar('ty', s[9], global_step=total_step + steps)
                        writer.add_scalar('tz', s[10], global_step=total_step + steps)
